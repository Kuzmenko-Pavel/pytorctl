#!/usr/bin/python
# StatsSupport.py - functions and classes useful for calculating
# stream/circuit statistics
...
class BandwidthStats:
    ...
    def _exp(self):  # Weighted avg
        ...
        for i in range(len(self.byte_list)):
            EX += (self.byte_list[i] * self.byte_list[i]
...
class StatsRouter(TorCtl.Router):
    ...
    def __init__(self, router):  # Promotion constructor :)
        ...
        self.refcount += 1
        plog("DEBUG", "Stats refcount " +
             str(self.refcount) + " for " + self.idhex)
    ...
    def strm_bw_ratio(self):
        ...
        bw = self.bwstats.mean
        if StatsRouter.global_strm_mean == 0.0:
            return 0
        else:
            return (1.0 * bw) / StatsRouter.global_strm_mean

    def circ_fail_rate(self):
        if self.circ_chosen == 0:
            return 0
        return (1.0 * self.circ_failed) / self.circ_chosen

    def strm_fail_rate(self):
        if self.strm_chosen == 0:
            return 0
    ...
    def current_uptime(self):
        if self.became_active_at:
            ret = (self.total_active_uptime +
                   (time.time() - self.became_active_at))
        else:
            ret = self.total_active_uptime
        if ret == 0:
            return 0.000005  # eh..
    ...
    def sanity_check(self):
        ...
        def check_reasons(reasons, expected, which, rtype):
            ...
            for rs in reasons.keys():
                if re.search(r"^" + which, rs):
                    count += reasons[rs]
            if count != expected:
                plog("ERROR", "Mismatch " + which + " " +
                     rtype + " for " + self.nickname)
        check_reasons(self.reason_suspected,
                      self.strm_suspected, "STREAM", "susp")
        check_reasons(self.reason_suspected,
                      self.circ_suspected, "CIRC", "susp")
        ...
        if self.hibernated_at:
            tot_hib_time += now - self.hibernated_at
        if self.became_active_at:
...
class StatsHandler(PathSupport.PathBuilder):
    ...
    def run_zbtest(self):  # Unweighted z-test
        ...
        if n == 0:
            return (0, 0)
        avg = reduce(lambda x, y: x + y.bwstats.mean,
                     self.sorted_r, 0) / float(n)

        def notlambda(x, y):
            if y.bwstats.mean <= 0:
                return x + 0
            else:
                return x + (y.bwstats.mean - avg) * (y.bwstats.mean - avg)
    ...
    def run_zrtest(self):  # Unweighted z-test
        ...
        if n == 0:
            return (0, 0)
        avg = reduce(lambda x, y: x + y.bw_ratio(),
                     self.sorted_r, 0) / float(n)
        ...
        def notlambda(x, y):
            if y.bw_ratio() <= 0:
                ...
                return x + (y.bw_ratio() - avg) * (y.bw_ratio() - avg)
        stddev = math.sqrt(reduce(notlambda, self.sorted_r, 0) / float(n))
        if not stddev:
            return (avg, stddev)
        for r in self.sorted_r:
    ...
    def write_routers(self, f, rlist, name):
        ...
        for r in rlist:
            ...
            if r.circ_chosen + r.strm_chosen > 0:
                f.write(str(r))
  ...
  P=Percentile Rank       U=Uptime (h)\n"""
    ...
    def write_ratios(self, filename):
        ...
        strm_bw_ratio.sort(lambda x, y: cmp(
            x.strm_bw_ratio(), y.strm_bw_ratio()))
        for r in strm_bw_ratio:
            if r.circ_chosen == 0:
                continue
            f.write(r.idhex + "=" + r.nickname + "\n  ")
            f.write("SR=" + str(round(r.strm_bw_ratio(), 4)) + " AR=" + str(round(r.adv_ratio(), 4)) + " BRR=" + str(round(r.bw_ratio_ratio(), 4)) + " CSR=" + str(round(r.circ_suspect_ratio(), 4)) + " CFR=" + str(round(r.circ_fail_ratio(), 4)) + " SSR=" + str(round(
                r.strm_suspect_ratio(), 4)) + " SFR=" + str(round(r.strm_fail_ratio(), 4)) + " CC=" + str(r.circ_chosen) + " SC=" + str(r.strm_chosen) + " U=" + str(round(r.current_uptime() / 3600, 1)) + " P=" + str(round((100.0 * r.avg_rank()) / len(self.sorted_r), 1)) + "\n")
        f.close()
    ...
    def write_stats(self, filename):
        ...
        for r in self.sorted_r:
            for rsn in r.reason_failed:
                if rsn not in self.failed_reasons:
                    plog("ERROR", "Router " + r.idhex +
                         " w/o reason " + rsn + " in fail table")
                elif r not in self.failed_reasons[rsn].rlist:
                    plog("ERROR", "Router " + r.idhex +
                         " missing from fail table")
            for rsn in r.reason_suspected:
                if rsn not in self.suspect_reasons:
                    ...
                    plog("ERROR", "Router " + r.idhex +
                         " missing from suspect table")

        # Sanity check the lists the other way
        for rsn in self.failed_reasons.values():
            rsn._verify_failed()
        for rsn in self.suspect_reasons.values():
        ...
        StatsRouter.global_strm_mean = avg
        StatsRouter.global_strm_dev = dev
        f.write("\n\nBW stats: u=" + str(round(avg, 1)) +
                " s=" + str(round(dev, 1)) + "\n")

        (avg, dev) = self.run_zrtest()
        StatsRouter.global_ratio_mean = avg
        StatsRouter.global_ratio_dev = dev
        f.write("BW ratio stats: u=" + str(round(avg, 1)) +
                " s=" + str(round(dev, 1)) + "\n")
        ...
        # Extend times
        n = 0.01 + reduce(lambda x, y: x +
                          (y.avg_extend_time() > 0), self.sorted_r, 0)
        avg_extend = reduce(
            lambda x, y: x + y.avg_extend_time(), self.sorted_r, 0) / n
        ...
        fail_rate.sort(lambda x, y: cmp(
            y.failed_per_hour(), x.failed_per_hour()))
    ...
    def circ_status_event(self, c):
        if c.circ_id in self.circuits:
            ...
            if c.status == "LAUNCHED":
                ...
                try:
                    if r_ext[0] != '$':
                        r_ext = self.name_to_key[r_ext]
                    self.routers[r_ext[1:]].total_extend_time += delta
                    self.routers[r_ext[1:]].total_extended += 1
                except KeyError as e:
                    traceback.print_exc()
                    plog("WARN", "No key " + str(e) + " for " +
                         r_ext + " in dict:" + str(self.name_to_key))
                ...
                for r in self.circuits[c.circ_id].path[:start_f]:
                    ...
                    if reason not in self.suspect_reasons:
                        self.suspect_reasons[
                            reason] = SuspectRouterList(reason)
    ...
    def stream_status_event(self, s):
        if s.strm_id in self.streams and not self.streams[s.strm_id].ignored:
            ...
            if s.reason:
                ...
                lreason = "NONE"
            if s.remote_reason:
                rreason = s.remote_reason
            else:
                rreason = "NONE"
            reason = s.event_name + ":" + s.status + ":" + lreason + \
                ":" + rreason + ":" + self.streams[s.strm_id].kind
            circ = self.streams[s.strm_id].circ
            if not circ:
                circ = self.streams[s.strm_id].pending_circ
            if (s.status in ("DETACHED", "FAILED", "CLOSED", "SUCCEEDED")
                    and not s.circ_id):
                # XXX: REMAPs can do this (normal). Also REASON=DESTROY (bug?)
                if circ:
                    plog("INFO", "Stream " + s.status + " of " + str(s.strm_id) +
                         " gave circ 0.  Resetting to stored circ id: " + str(circ.circ_id))
                ...
                #  plog("NOTICE", "Stream "+str(s.strm_id)+" detached with "+s.reason)
                    ...
                    plog("WARN", "Stream " + str(s.strm_id) +
                         " detached from no known circuit with reason: " + str(s.reason))
            ...
            if s.status == "DETACHED":
                ...
                self.strm_count += 1
                for r in self.circuits[s.circ_id].path:
                    r.strm_chosen += 1
                self.strm_failed += 1
                ...
                if self.streams[s.strm_id].failed:
                    reason = self.streams[
                        s.strm_id].failed_reason + ":" + lreason + ":" + rreason
                ...
                if (not self.streams[s.strm_id].failed
                    ...
                    if self.streams[s.strm_id].attached_at:
                        lifespan = self.streams[
                            s.strm_id].lifespan(s.arrived_at)
