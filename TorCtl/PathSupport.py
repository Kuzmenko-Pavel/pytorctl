...
class PathRestrictionList(MetaPathRestriction):
    ...
    def del_restriction(self, RestrictionClass):
        ...
        self.restrictions = [
            r for r in self.restrictions if not isinstance(r, RestrictionClass)]
...
class NodeGenerator:
    ...
    def rewind(self):
        ...
        if not self.routers:
            plog("NOTICE", "No routers left after restrictions applied: " +
                 str(self.rstr_list))
    ...
    def rebuild(self, sorted_r=None):
        ...
        self.rstr_routers = [
            r for r in self.sorted_r if self.rstr_list.r_is_ok(r)]
...
class PercentileRestriction(NodeRestriction):
    ...
    def r_is_ok(self, r):
        ...
        if r.list_rank < len(self.sorted_r) * self.pct_skip / 100:
            return False
        elif r.list_rank > len(self.sorted_r) * self.pct_fast / 100:
            return False
...
class UptimeRestriction(NodeRestriction):
    ...
    def r_is_ok(self, r):
        ...
        if self.min_uptime and r.uptime < self.min_uptime:
            return False
        if self.max_uptime and r.uptime > self.max_uptime:
            return False
        return True
...
class RankRestriction(NodeRestriction):
    ...
    def r_is_ok(self, r):
        ...
        if r.list_rank < self.rank_skip:
            return False
        elif r.list_rank > self.rank_stop:
            return False
...
class VersionRangeRestriction(NodeRestriction):
    ...
    def __init__(self, gr_eq, less_eq=None):
        ...
        if less_eq:
            self.less_eq = TorCtl.RouterVersion(less_eq)
        else:
            self.less_eq = None
...
class ExactUniformGenerator(NodeGenerator):
    ...
    def generate(self):
        ...
        choices = [r for r in self.routers if r._generated[
            self.position] == min_gen]
        while choices:
...
class BwWeightedGenerator(NodeGenerator):
        ...
        if self.exit:
            ...
            if self.total_exit_bw < bw_per_hop:
                ...
                if self.total_exit_bw > 0:
                    self.exit_weight = (
                        (self.total_exit_bw - bw_per_hop) / self.total_exit_bw)
                else:
                    self.exit_weight = 0
        ...
        if self.guard:
            ...
            if self.total_guard_bw < bw_per_hop:
                ...
                if self.total_guard_bw > 0:
                    self.guard_weight = (
                        (self.total_guard_bw - bw_per_hop) / self.total_guard_bw)
                else:
                    self.guard_weight = 0
    ...
    def generate(self):
        while True:
            ...
            for r in self.routers:
                # Below zero here means next() -> choose a new random
                # int+router
                if i < 0:
                    break
                bw = r.bw
...
class SelectionManager(BaseSelectionManager):
    ...
    def _reconfigure(self, consensus=None):
        ...
        if self.use_guards:
            entry_flags = ["Guard", "Running", "Fast"]
        else:
            entry_flags = ["Running", "Fast"]
        ...
        if self.geoip_config:
            ...
            if self.geoip_config.entry_country:
                entry_rstr.add_restriction(
                    CountryRestriction(self.geoip_config.entry_country))
            if self.geoip_config.middle_country:
                mid_rstr.add_restriction(CountryRestriction(
                    self.geoip_config.middle_country))
            if self.geoip_config.exit_country:
                self.exit_rstr.add_restriction(
                    CountryRestriction(self.geoip_config.exit_country))

            # Excluded countries
            if self.geoip_config.excludes:
                plog("INFO", "Excluded countries: " +
                     str(self.geoip_config.excludes))
                if len(self.geoip_config.excludes) > 0:
                    entry_rstr.add_restriction(
                        ExcludeCountriesRestriction(self.geoip_config.excludes))
            ...
            # Specify max number of continent crossings, None means
            # UniqueContinents
            if self.geoip_config.continent_crossings == None:
                self.path_rstr.add_restriction(UniqueContinentRestriction())
            else:
                self.path_rstr.add_restriction(ContinentRestriction(
                    self.geoip_config.continent_crossings))
            # Should even work in combination with continent crossings
            if self.geoip_config.ocean_crossings != None:
                self.path_rstr.add_restriction(
                    OceanPhobicRestriction(self.geoip_config.ocean_crossings))
        ...
        if self.order_exits:
            ...
            exitgen = BwWeightedGenerator(
                sorted_r, self.exit_rstr, self.pathlen, exit=True)
    ...
    def set_exit(self, exit_name):
        ...
        if not self.exit_id:
            ...
            plog("NOTICE", "Requested downed exit " + str(self.exit_id) +
                 " (bw: " + str(e.bw) + ", flags: " + str(e.flags) + ")")
            self.bad_restrictions = True
        elif self.consensus.routers[self.exit_id[1:]].deleted:
            e = self.consensus.routers[self.exit_id[1:]]
            plog("NOTICE", "Requested deleted exit " + str(self.exit_id) + " (bw: " + str(e.bw) +
                 ", flags: " + str(e.flags) + ", Down: " + str(e.down) + ", ref: " + str(e.refcount) + ")")
            self.bad_restrictions = True
        else:
    ...
    def set_target(self, ip, port):
        ...
        if self.__ordered_exit_gen:
            self.__ordered_exit_gen.set_port(port)
        ...
        if self.geoip_config and self.geoip_config.echelon:
            ...
            if c:
                ...
                plog(
                    "INFO", "[Echelon] Could not determine destination country of IP " + ip)
...
class PathBuilder(TorCtl.ConsensusTracker):
    ...
    def __init__(self, c, selmgr, RouterClass=TorCtl.Router,
        ...
        plog("INFO", "Read " + str(len(self.sorted_r)) +
             "/" + str(len(self.ns_map)) + " routers")
    ...
    def heartbeat_event(self, event):
        ...
        if PathBuilder.is_urgent_event(event):
            return
        ...
        for strm in self.streams.values():
            if not strm.ignored:
                try:
                    ...
                    plog("NOTICE", "Error closing stream " +
                         str(strm.strm_id) + ": " + str(e))
    ...
    def close_circuit(self, id):
        ...
        if self.circuits[id].requested_closed:
            return
        self.circuits[id].requested_closed = True
        try:
            self.c.close_circuit(id)
        except TorCtl.ErrorReply as e:
    ...
    def attach_stream_any(self, stream, badcircs):
            ...
            for key in list(self.circuits.keys()):
                if (not self.circuits[key].dirty
                    ...
                    plog("WARN", "New nym called, destroying circuit " + str(key)
                         + " with " +
                         str(len(self.circuits[key].pending_streams))
                         + " pending streams")
                    unattached_streams.extend(
        ...
        for circ in self.circuit_list():
            ...
            try:
                ...
                self.last_exit = None
                # Kill this stream
                plog("WARN", "Closing impossible stream " +
                     str(stream.strm_id) + " (" + str(e) + ")")
                try:
                    # END_STREAM_REASON_EXITPOLICY
                    self.c.close_stream(stream.strm_id, "4")
                except TorCtl.ErrorReply as e:
    ...
    def circ_status_event(self, c):
        output = [str(time.time() - c.arrived_at), c.event_name, str(c.circ_id),
                  c.status]
        if c.path:
            output.append(",".join(c.path))
        if c.reason:
            output.append("REASON=" + c.reason)
        if c.remote_reason:
            output.append("REMOTE_REASON=" + c.remote_reason)
        ...
        if c.status == "EXTENDED":
            ...
            for r in circ.path:
                r.refcount -= 1
                plog("DEBUG", "Close refcount " +
                     str(r.refcount) + " for " + r.idhex)
            ...
            for stream in circ.pending_streams:
                ...
                if not circ.built:
                    ...
                    plog("NOTICE", "Waiting on Tor to hint about stream " +
                         str(stream.strm_id) + " on closed circ " + str(circ.circ_id))
        elif c.status == "BUILT":
    ...
    def stream_status_event(self, s):
        ...
        if s.reason:
            output.append("REASON=" + s.reason)
        if s.remote_reason:
            output.append("REMOTE_REASON=" + s.remote_reason)
        if s.purpose:
            output.append("PURPOSE=" + s.purpose)
        if s.source_addr:
            output.append("SOURCE_ADDR=" + s.source_addr)
        ...
        if s.status == "NEW" or s.status == "NEWRESOLVE":
            ...
            if s.circ_id == 0:
                self.streams[s.strm_id] = Stream(
                    s.strm_id, s.target_host, s.target_port, s.status)
            ...
            if s.strm_id not in self.streams:
                plog("WARN", "Detached stream " +
                     str(s.strm_id) + " not found")
            ...
            if not s.circ_id:
                if s.reason == "TIMEOUT" or s.reason == "EXITPOLICY":
                    plog("NOTICE", "Stream " + str(s.strm_id) +
                         " detached with " + s.reason)
                else:
                    plog("WARN", "Stream " + str(s.strm_id) +
                         " detached from no circuit with reason: " + str(s.reason))
            else:
            ...
            if s.circ_id and self.streams[s.strm_id].pending_circ.circ_id != s.circ_id:
                ...
                if s.circ_id in self.circuits:
                    ...
                    plog("NOTICE", "Stream " + str(s.strm_id) +
                         " has unknown circuit: " + str(s.circ_id))
            else:
                self.streams[s.strm_id].circ = self.streams[
                    s.strm_id].pending_circ
            self.streams[s.strm_id].pending_circ.pending_streams.remove(self.streams[
                                                                        s.strm_id])
            self.streams[s.strm_id].pending_circ = None
            ...
            if not s.circ_id:
                if s.reason == "TIMEOUT" or s.reason == "EXITPOLICY":
                    plog("NOTICE", "Stream " + str(s.strm_id) +
                         " " + s.status + " with " + s.reason)
                else:
                    plog("WARN", "Stream " + str(s.strm_id) + " " + s.status +
                         " from no circuit with reason: " + str(s.reason))
            ...
            if s.status == "FAILED":
                ...
                if s.circ_id in self.circuits:
                    self.circuits[s.circ_id].dirty = True
                elif s.circ_id != 0:
                    plog("WARN", "Failed stream " + str(s.strm_id) +
                         " on unknown circ " + str(s.circ_id))
                return
...
class CircuitHandler(PathBuilder):
    ...
    def circ_status_event(self, c):
        ...
        if c.status == "EXTENDED":
            ...
            extend_time = c.arrived_at - \
                self.circuits[c.circ_id].last_extended_at
...
def do_gen_unit(gen, r_list, weight_bw, num_print):
    ...
    for r in copy_rlist:
        ...
        if not gen.rstr_list.r_is_ok(r):
            continue
        flag = ""
        ...
        if "Guard" in r.flags:
            flag += "G"
        print(str(r.list_rank) + ". " + r.nickname + " " + str(r.bw / 1024.0) +
              "/" + str(bw / 1024.0) + ": " + str(r.chosen) + ", " + flag)
        i += 1
        if i > num_print:
            break
...
def do_unit(rst, r_list, plamb):
    ...
    for r in r_list:
        if rst.r_is_ok(r):
            print(r.nickname + " " + plamb(r) + "=" +
                  str(rst.r_is_ok(r)) + " " + str(r.bw))
            if r.bw > 400000:
                above_i = above_i + 1
                above_bw += r.bw
            else:
...
if __name__ == '__main__':
    ...
    for i in range(len(sorted_rlist)):
    ...
    do_unit(FlagsRestriction(["Guard"], []),
            sorted_rlist, lambda r: " ".join(r.flags))
    do_unit(FlagsRestriction(["Fast"], []),
            sorted_rlist, lambda r: " ".join(r.flags))
    ...
    do_unit(ConserveExitsRestriction(), sorted_rlist,
            lambda r: " ".join(r.flags))
    do_unit(FlagsRestriction([], ["Valid"]),
            sorted_rlist, lambda r: " ".join(r.flags))

    do_unit(IdHexRestriction("$FFCB46DB1339DA84674C70D7CB586434C4370441"),
            sorted_rlist, lambda r: r.idhex)

    rl = [AtLeastNNodeRestriction([ExitPolicyRestriction("255.255.255.255", 80), ExitPolicyRestriction(
        "255.255.255.255", 443), ExitPolicyRestriction("255.255.255.255", 6667)], 2), FlagsRestriction([], ["BadExit"])]
