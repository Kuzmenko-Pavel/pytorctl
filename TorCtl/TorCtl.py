        ...
        if ip_mask == "*":
            ...
            if not "/" in ip_mask:
                ...
                if ipaddress_re.match(mask):
                    self.netmask = struct.unpack(
                        ">I", socket.inet_aton(mask))[0]
                else:
...
class RouterVersion:
    ...
    def __init__(self, version):
        if version:
            ...
            self.version = int(v[0]) * 0x1000000 + int(v[1]) * \
                0x10000 + int(v[2]) * 0x100 + int(v[3])
...
class Router:
        ...
        for line in desc:
            ...
            if kw == "accept":
                ...
                if ("Running" in ns.flags):
                    plog("INFO", "Hibernating router " + ns.nickname +
                         " is running, flags: " + " ".join(ns.flags))

        if router != ns.nickname:
            plog("INFO", "Got different names " + ns.nickname + " vs " +
                         router + " for " + ns.idhex)
        if not bw_observed and not dead and ("Valid" in ns.flags):
            plog("INFO", "No bandwidth for live router " +
                 ns.nickname + ", flags: " + " ".join(ns.flags))
            dead = True
    ...
    def update_to(self, new):
        ...
        if self.idhex != new.idhex:
            plog("ERROR", "Update of router " +
                 self.nickname + "changes idhex!")
        for i in new.__dict__.keys():
            if i == "refcount" or i == "_generated":
                continue
...
class Connection:
    ...
    def get_auth_type(self):
        ...
        if self._authType:
            ...
            if authInfo.startswith("AUTH METHODS=NULL"):
                ...
                # control-spec?)
                raise IOError(
                    "Unrecognized authentication type: %s" % authInfo)
    ...
    def _read_reply(self):
        ...
        while 1:
            ...
            if tp == "-":
                ...
                raise ProtocolError(
                    "Badly formatted reply line: unknown type %r" % tp)
            else:
                more = []
                while 1:
                    line = self._s.readline()
    ...
    def authenticate(self, secret=""):
        ...
        try:
            ...
            # string
            if issue:
                raise IOError("Failed to read authentication cookie (%s): %s" % (
                    issue, self._cookiePath))
            else:
                raise IOError("Failed to read authentication cookie: %s" % exc)
    ...
    def get_address_mappings(self, type="all"):
        ...
        for l in lines:
            # print "|"+str(l)+"|"
            if len(l) == 0:
                continue  # Skip last line.. it's empty
    ...
    def get_router(self, ns):
        ...
        if fp_base64 != ns.orhash:
            plog("INFO", "Router descriptor for " + ns.idhex + " does not match ns fingerprint (NS @ " +
                 str(ns.updated) + " vs Desc @ " + str(r.published) + ")")
            return None
        else:
            return r
    ...
    def attach_stream(self, streamid, circid, hop=None):
        ...
        if hop:
            self.sendAndRecv("ATTACHSTREAM %d %d HOP=%d\r\n" %
                             (streamid, circid, hop))
            plog("DEBUG", "Attaching stream: " + str(streamid) +
                 " to hop " + str(hop) + " of circuit " + str(circid))
        else:
            self.sendAndRecv("ATTACHSTREAM %d %d\r\n" % (streamid, circid))
            plog("DEBUG", "Attaching stream: " +
                 str(streamid) + " to circuit " + str(circid))
    ...
    def post_descriptor(self, desc):
        self.sendAndRecv(
            "+POSTDESCRIPTOR purpose=controller\r\n%s" % escape_dots(desc))
...
def parse_ns_body(data):
    ...
    for nsline in nsgroups:
        ...
        m = re.match(
            r"(\S+)\s(\S+)\s(\S+)\s(\S+\s\S+)\s(\S+)\s(\d+)\s(\d+)", nsline)
        w = re.search(r"^w Bandwidth=(\d+)", nsline, re.M)
        if w:
            nslist.append(NetworkStatus(
                *(m.groups() + (flags,) + (int(w.group(1)) * 1000,))))
        else:
...
class EventListener(EventSink):
    ...
    def _sabotage(self):
        raise TorCtlError(
            "Error: Do not extend from EventListener directly! Use Pre, Post or DualEventListener instead.")
...
class EventHandler(EventSink):
    ...
    def _decode1(self, body, data):
        ...
        if evtype == "CIRC":
            ...
            if remote:
                remote = remote[15:]
            event = CircuitEvent(evtype, ident, status,
                                 path, purpose, reason, remote)
        elif evtype == "STREAM":
            #plog("DEBUG", "STREAM: "+body)
            m = re.match(
                r"(\S+)\s+(\S+)\s+(\S+)\s+(\S+)?:(\d+)(\sREASON=\S+)?(\sREMOTE_REASON=\S+)?(\sSOURCE=\S+)?(\sSOURCE_ADDR=\S+)?(\s+PURPOSE=\S+)?", body)
            if not m:
            ...
            m = re.match(
                r"(\S+)\s+(\S+)(\sAGE=\S+)?(\sREAD=\S+)?(\sWRITTEN=\S+)?(\sREASON=\S+)?(\sNCIRCS=\S+)?", body)
            if not m:
...
class ConsensusTracker(EventHandler):
    ...
    def _read_routers(self, nslist):
        ...
        removed_idhexes.update(
            set([r.idhex for r in [r for r in routers if r.down]]))

        for i in removed_idhexes:
            if i not in self.routers:
                continue
        ...
        for i in range(len(self.sorted_r)):
            self.sorted_r[i].list_rank = i
    ...
    def _sanity_check(self, list):
        if len(self.routers) > 1.5 * self.consensus_count:
            plog("WARN", "Router count of " + str(len(self.routers)) +
                 " exceeds consensus count " + str(self.consensus_count) + " by more than 50%")

        if len(self.ns_map) < self.consensus_count:
            plog("WARN", "NS map count of " + str(len(self.ns_map)) +
                 " is below consensus count " + str(self.consensus_count))

        downed = [r for r in list if r.down]
        for d in downed:
            plog("WARN", "Router " + d.idhex + " still present but is down. Del: " +
                 str(d.deleted) + ", flags: " + str(d.flags) + ", bw: " + str(d.bw))

        deleted = [r for r in list if r.deleted]
        for d in deleted:
            plog("WARN", "Router " + d.idhex + " still present but is deleted. Down: " +
                 str(d.down) + ", flags: " + str(d.flags) + ", bw: " + str(d.bw))

        zero = [r for r in list if r.refcount ==
                0 and r.__class__.__name__ == "StatsRouter"]
        for d in zero:
            plog("WARN", "Router " + d.idhex + " has refcount 0. Del:" + str(d.deleted) +
                 ", Down: " + str(d.down) + ", flags: " + str(d.flags) + ", bw: " + str(d.bw))
    ...
    def new_desc_event(self, d):
        ...
        for i in d.idlist:
            ...
            try:
                ...
                plog("WARN", "Error reply for " +
                     i + " after NEWDESC: " + str(e))
                continue
            if not r:
        ...
        if update:
            self.sorted_r = [r for r in iter(
                self.routers.values()) if not r.down]
            self.sorted_r.sort(lambda x, y: cmp(y.bw, x.bw))
            for i in range(len(self.sorted_r)):
                self.sorted_r[i].list_rank = i
...
class DebugEventHandler(EventHandler):
    ...
    def or_conn_status_event(self, orconn_event):
        if orconn_event.age:
            age = "AGE=" + str(orconn_event.age)
        else:
            age = ""
        if orconn_event.read_bytes:
            read = "READ=" + str(orconn_event.read_bytes)
        else:
            read = ""
        if orconn_event.wrote_bytes:
            wrote = "WRITTEN=" + str(orconn_event.wrote_bytes)
        else:
            wrote = ""
        if orconn_event.reason:
            reason = "REASON=" + orconn_event.reason
        else:
            reason = ""
        if orconn_event.ncircs:
            ncircs = "NCIRCS=" + str(orconn_event.ncircs)
        else:
            ncircs = ""
    ...
    def bandwidth_event(self, bw_event):
        print(bw_event.event_name + " " +
              str(bw_event.read) + " " + str(bw_event.written))
