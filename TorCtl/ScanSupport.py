...
class ScanHandler(PathSupport.PathBuilder):
    ...
    def percent_to_rank(self, pct):
        ...
        def notlambda(this):
            cond.acquire()
            # lol moar haxx
            cond._rank = int(round((pct * len(this.sorted_r)) / 100.0, 0))
            cond.notify()
            cond.release()
        cond.acquire()
        self.schedule_low_prio(notlambda)
...
class SQLScanHandler(ScanHandler):
    ...
    def write_sql_stats(self, rfilename=None, stats_filter=None):
        if not rfilename:
            rfilename = "./data/stats/sql-" + \
                time.strftime("20%y-%m-%d-%H:%M:%S")
        cond = threading.Condition()
    ...
    def wait_for_consensus(self):
        ...
        def notlambda(this):
            if this.sql_consensus_listener.last_desc_at \
                ...
                plog("INFO", "Waiting on consensus result: " +
                     str(this.run_all_jobs))
